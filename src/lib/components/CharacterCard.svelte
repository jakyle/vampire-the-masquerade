<script lang="ts">
  ...
  export let selectedSkill: keyof CharacterInfo | "";
  export let selectedAttribute: keyof CharacterInfo | "";
  ...
  const selectWithId = (id: string) =>
    dispatchSelectCharacter("selectCharacter", id);
...
>
  ...
  <table
    class="h-14 text-white [&>tr>*]:w-1/2 [&>tr>*]:border [&>tr>*]:border-stone-500"
  >
    <tr>
  ...
  <div class="flex-none">
    <Slider
      on:input={updateHunger}
      bind:value={character.hunger}
      label="Hunger"
    />
  </div>
</div>
